...
const Profile = () => {
  ...
  useEffect(() => {
    console.log("peaky");
    console.log(typeof defaultProfilePic);
    const fetchUserData = async () => {
      try {
        ...
        setEmail(data.email || '');
        console.log("users profile pic path")
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch('http://localhost:8000/users-login-api/users-logout', {
        method: 'POST',
        credentials: 'include', // Ensure the cookie is sent and cleared
      });
  ...
  const handleProfileImageChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const fetchUserDataUrvashi = async () => {
        try {
            const response = await fetch('http://localhost:8000/profile-api/me', {
              credentials: 'include', // Include cookies in the request
            });
            ...
            if (!response.ok) {
              ...
              const userurvashi = await response.json();
              setProfilePicture(userurvashi.profilePicture); // Preview the image
            }
        }catch(err)
        {
          console.log("error in handlePorfileImageChange");
        }
      
      }

      fetchUserDataUrvashi();
  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (newProfilePicture) {
      formData.append('profileImageForMulter', newProfilePicture);
    }
    ...
    try {
      ...
      const updatedData = await response.json(); // Assuming the server returns updated user data
      setProfilePicture(updatedData.url || defaultProfilePic); // Update the state
  ...
  return (
    <div className="flex flex-col justify-start items-start h-screen bg-primaryBlack text-primaryWhite px-8 pt-20">
      ...
      <div className="w-full mb-8 bg-primaryWhite text-primaryBlack p-6 rounded">
        ...
        <input
          type="file"
          name="profileImageForMulter"
          id="profileImageInput"
          accept="image/*"
          style={{ display: 'none' }}
          onChange={handleProfileImageChange}
        />
        <form onSubmit={handleUpdateProfile} className="w-full mb-8" encType='multipart/form-data'>
          <div className="mb-4">
